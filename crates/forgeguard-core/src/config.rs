...
pub struct ScanConfig {
    ...
    /// LCOV report generated by the project's existing test tooling.
    #[serde(default)]
...
impl ForgeGuardConfig {
    ...
    fn load_from_path(path: &Path) -> Result<Self> {
        ...
        {
            bail!("scan.min_changed_coverage must be between 0 and 100");
        }
        if config.scan.min_changed_coverage.is_some() && config.scan.coverage_report.is_none() {
            bail!("scan.coverage_report is required when scan.min_changed_coverage is set");
        }
        Ok(config)
    }
